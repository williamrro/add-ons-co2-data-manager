...
export class ManageFiltersComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const clientCode =
      selectedClients && selectedClients.length > 0
        ? selectedClients[0][utilService.DROPDOWN_KEY]
        : "";
  ...
  formatAllFiltersToManage() {
    this.allFiltersToManage = [...this.allFilters].map((filter: any) => {
      return {
        ...filter,
        disabled: this.selectedFilterToManage.some(
          (itm: any) => itm.key === filter.key
        ),
      };
    });
  }
  ...
  onDeSelectFilter(filterKey: string) {
    ...
    this.selectedFilterToManage = this.selectedFilterToManage.filter(
      (itm: any) => itm.key !== filterKey
    );
    this.formatAllFiltersToManage();
  }
