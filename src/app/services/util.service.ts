...
export class UtilService {
  ...
  formatT4SearchPayload(searchParams: any) {
    ...
    for (let groupKey in searchParams) {
      const group = searchParams[groupKey];
      filteredResult[groupKey] = {};

      for (let key in group) {
        const items = group[key];

        if (Array.isArray(items) && items.length > 0) {
          // Extract the 'id' values from objects
          const ids = items
            .map((item) => item.id)
            .filter((id) => id !== undefined);
          if (ids.length > 0) {
            filteredResult[groupKey][key] = ids;
          }
        }
      }
    }
  ...
  resetDropdownPosition() {
    setTimeout(() => {
      try {
        ...
        const openedDropdownElems = Array.from(
          document.querySelectorAll("div.dropdown-list:not([hidden])")
        );

        if (openedDropdownElems.length > 0) {
          openedDropdownElems.forEach((elem: any) => {
            // Rect of actual dropdown element
            const dropdownHeaderElemRect =
            ...
            const valuesListToOpenTop =
              dropdownHeaderElemRect.top +
              dropdownHeaderElemRect.height +
              valuesListMargin;
            // Check if sum of opened dropdown values list element top and height exceeds available body height
            if (
              valuesListToOpenTop + openedDropdownElemRect.height >
              bodyHeight
            ) {
